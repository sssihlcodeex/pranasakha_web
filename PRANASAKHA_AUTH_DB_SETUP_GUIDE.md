...
## Lifetime QR Pass / Checkpoint Setup

The project now uses an opaque, random lifetime QR token rather than placing a person's data in the QR code.
...
This applies `007_lifetime_qr_passes.sql` and creates:

- `passes`
- `scan_logs`
- `checkpoints`

The migration also creates an initial checkpoint record for Gate, Canteen, Mandir, and Mobile Camp, and issues lifetime passes for already-approved healthcare volunteers that do not already have one.

### QR dependency

The web backend renders QR images using the `qrcode` npm package. After replacing the project, run:

```bash
npm install
```

The QR payload is only the opaque token, for example:
...
POST /api/passes/:id/suspend
POST /api/passes/resolve
GET  /api/scans?user_id=
```

`POST /api/passes/resolve` takes:

```json
{
  "token": "PSK-...",
  "checkpoint_type": "gate",
  "location": "optional scanner location"
}
```

The server responds with only the fields allowed for that checkpoint type.

### Directorate approval

When a Doctor/Healthcare Volunteer application changes to `approved`, the backend automatically creates the lifetime pass if the user does not already have an active/suspended pass.

### Web Doctor Dashboard

The existing `Digital medical pass` dashboard section now displays the large QR, the short fallback code, the person's category/institution, and recent scan history after approval.

### Admin

The Admin dashboard includes a QR Pass Management section for viewing pass status and suspending/revoking passes. Revocation is server-side and immediately makes the token invalid at every checkpoint.

### Mobile app

An additive `mobile/` Expo app scaffold is included. It uses `expo-camera` for QR scanning and `expo-secure-store` for authentication tokens. AsyncStorage is used for non-sensitive queued scan state. Expo documents `CameraView` barcode scanning, SecureStore for encrypted device storage, AsyncStorage for unencrypted persistent app state, and EAS Build for cloud compilation/distribution. 

For the Android preview APK build:
...
The mobile app currently implements the doctor virtual-ID screen and online scanner/queue flow. A fully cryptographically verified offline allow-list should be completed as a separate security phase rather than shipping a symmetric secret inside the mobile app.
